...
theorem iterate_map_mul (f : M →* M) (n : ℕ) (x y) :
  f^[n] (x * y) = (f^[n] x) * (f^[n] y) :=
conjugates₂.iterate f.map_mul n x y
...
@[simp]
theorem iterate_map_sub [add_group G] (f : G →+ G) (n : ℕ) (x y) :
  f^[n] (x - y) = (f^[n] x) - (f^[n] y) :=
conjugates₂.iterate f.map_sub n x y

theorem iterate_map_smul [add_monoid M] (f : M →+ M) (n m : ℕ) (x : M) :
  f^[n] (m •ℕ x) = m •ℕ (f^[n] x) :=
f.to_multiplicative.iterate_map_pow x n m

theorem iterate_map_gsmul [add_group G] (f : G →+ G) (n : ℕ) (m : ℤ) (x : G) :
  f^[n] (m •ℤ x) = m •ℤ (f^[n] x) :=
